...
class VolumeDistillationLoss(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            use_l1: Use L1 loss
            use_mse: Use MSE loss (better for PSNR optimization)
            use_ssim: Use SSIM loss (computationally expensive for 3D)
            use_mask: Only compute loss in high-density regions
            mask_threshold: Density threshold for masking
        """
        super().__init__()
    ...
    def forward(
        self, teacher_volume: torch.Tensor, student_volume: torch.Tensor
        ...
        # Masked distillation: only compute loss in high-density regions
        mask = None
        if self.use_mask:
            # Mask = regions where teacher OR student has significant density
            threshold = self.mask_threshold
            mask = (teacher_volume > threshold) | (student_volume > threshold)
            mask = mask.float()

        losses = {}

        # KL divergence loss (soft labels) - not masked (operates on full distribution)
        if self.use_kl:
        ...
        if self.use_l1:
            if mask is not None:
                # Masked L1: only in high-density regions
                diff = torch.abs(student_volume - teacher_volume) * mask
                l1_loss = diff.sum() / (mask.sum() + 1e-8)
            else:
                l1_loss = F.l1_loss(student_volume, teacher_volume)
            losses["l1"] = l1_loss

        # MSE loss (better correlated with PSNR)
        if self.use_mse:
            if mask is not None:
                diff_sq = ((student_volume - teacher_volume) ** 2) * mask
                mse_loss = diff_sq.sum() / (mask.sum() + 1e-8)
            else:
                mse_loss = F.mse_loss(student_volume, teacher_volume)
            losses["mse"] = mse_loss
    ...
    def _kl_divergence_loss(
        self, teacher_vol: torch.Tensor, student_vol: torch.Tensor
    ) -> torch.Tensor:
        """Compute KL divergence between teacher and student volume distributions."""
        # Soften distributions with temperature
        teacher_soft = F.softmax(
            teacher_vol.view(teacher_vol.size(0), -1) / self.temperature, dim=-1
        )
        student_soft = F.log_softmax(
            student_vol.view(student_vol.size(0), -1) / self.temperature, dim=-1
        )
...
class ProjectionConsistencyLoss(nn.Module):
    ...
    def forward(
        self, student_projections: torch.Tensor, target_projections: torch.Tensor
    ) -> torch.Tensor:
        """
...
class SSIM3DApproximation(nn.Module):
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        ...
        for d in range(depth):
            ...
            sigma_x_sq = (
                F.conv2d(x_slice**2, window, padding=self.window_size // 2) - mu_x_sq
            )
            sigma_y_sq = (
                F.conv2d(y_slice**2, window, padding=self.window_size // 2) - mu_y_sq
            )
            sigma_xy = (
                F.conv2d(x_slice * y_slice, window, padding=self.window_size // 2)
                - mu_x_mu_y
            )
...
def create_distillation_loss(config: Dict) -> nn.Module:
    ...
    if loss_type == "volume":
        return VolumeDistillationLoss(
            ...
            use_mse=config.get("use_mse", True),
            use_ssim=config.get("use_ssim", False),
            use_mask=config.get("use_mask", False),
            mask_threshold=config.get("mask_threshold", 0.01),
        )
...
if __name__ == "__main__":
    ...
    dist_loss = VolumeDistillationLoss(
        temperature=2.0, alpha=0.7, use_kl=True, use_l1=True, use_ssim=False
    )
