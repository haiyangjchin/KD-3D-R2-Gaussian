...
class CNNDistillationTeacher:
    ...
    def __init__(self, cnn_model_path, device='cuda', target_depth=64, model_size='small'):
        """
        Args:
    ...
    def precompute_volume_from_scene(self, scene, scanner_cfg=None):
        ...
        if scanner_cfg is not None:
            ...
            if (current_d, current_h, current_w) != (target_d, target_h, target_w):
                ...
                teacher_volume = teacher_volume.unsqueeze(0).unsqueeze(0)  # [1, 1, D, H, W]
    ...
    def get_volume(self, scanner_cfg=None):
        ...
        if not hasattr(self, 'cached_volume') or self.cached_volume is None:
            raise ValueError("Teacher volume not precomputed. Call precompute_volume_from_scene first.")
...
def training_with_distillation(model_path, iteration, testing_iterations, scene, 
                             gaussians, opt, pipe, teacher_model, 
                             distillation_loss, progressive_distill, 
                             use_importance_sampling=False, view_sampler=None,
                             enhanced_regularization=False, scanner_cfg=None):
    """
    ...
    loss = {"total": torch.tensor(0.0, device=device), 
    ...
    if opt.lambda_tv > 0:
        ...
        bbox_min = torch.tensor(scanner_cfg["offOrigin"]) - torch.tensor(scanner_cfg["sVoxel"]) / 2
        bbox_max = torch.tensor(scanner_cfg["offOrigin"]) + torch.tensor(scanner_cfg["sVoxel"]) / 2
        tv_vol_center = bbox_min + tv_vol_sVoxel / 2 + (bbox_max - tv_vol_sVoxel - bbox_min) * torch.rand(3)
        ...
        if enhanced_regularization and IMPROVED_TRAINING_AVAILABLE:
            ...
            loss["standard"] = loss["standard"] + 0.02 * loss_depth  # lambda_depth = 0.02
    ...
    if use_importance_sampling and view_sampler is not None and IMPROVED_TRAINING_AVAILABLE:
        # Find view index
        view_idx = None
...
def train_with_distillation(model_path, iteration, testing_iterations, scene, gaussians, opt, pipe,
                           cnn_model_path=None, distillation_config=None, use_improvements=True,
                           output_path=None, checkpoint_iterations=[], saving_iterations=[]):
    """
    ...
    progress_bar = tqdm(range(0, opt.iterations), desc="Train with Distillation", leave=False)
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        loss, render_pkg = training_with_distillation(
            model_path, iteration, testing_iterations, scene, gaussians, opt, pipe,
            teacher, distill_loss, progressive_distill,
        ...
        if iteration < opt.densify_until_iter and False:
            ...
            if hasattr(opt, 'opacity_reset_interval'):
                ...
                if iteration % opt.densification_interval == 0 or iteration == opt.densify_from_iter:
                    gaussians.reset_opacity()
        ...
        if iteration % 10 == 0:
            ...
            if use_improvements and view_sampler is not None and iteration >= view_sampler.warmup_iters:
    ...
    print(f"  Final distillation weight: {progressive_distill.get_weight(opt.iterations):.3f}")
...
def main():
    ...
    parser.add_argument("--no_improvements", action="store_true",
    ...
    use_distillation = not args.no_distill and args.cnn_model and osp.exists(args.cnn_model)
