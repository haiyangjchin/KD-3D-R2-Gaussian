...
def evaluate_all_iterations(base_path="./distill_student_10k_walnut_tv"):
    """
    评估所有迭代的结果
    """
    results = {}
...
def save_results_to_csv(
    results, output_path="distill_evaluation_results_walnut_tv.csv"
):
    """
    将结果保存到CSV文件
    """
    if results:
        ...
        with open(md_path, "w") as f:
            f.write("# Walnut数据集蒸馏训练评估结果 (3D TV + 教师学生知识蒸馏)\n\n")
            f.write(f"数据集: data/real_dataset/cone_ntrain_25_angle_360/walnut\n")
            f.write(f"迭代次数: 10000\n")
            f.write(f"检查点: 2500, 5000, 7500, 10000\n")
            f.write(f"3D TV权重 (λ_tv): 0.1\n")
            f.write(
                f"教师模型: cnn_teacher_50epoch_v4/checkpoints/checkpoint_epoch_034.pth\n\n"
            )
