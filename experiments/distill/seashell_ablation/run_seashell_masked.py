#!/usr/bin/env python3
"""
Seashell Masked L1 Only 自蒸馏
去掉KL和MSE，只用masked L1，只在高密度区域蒸馏

V: masked L1 only, mw=0.5, w=5k, i=8 (和Q对照，只改loss)
W: masked L1 only, mw=0.3, w=5k, i=8 (更轻)
X: masked L1 only, mw=0.1, w=5k, i=8 (极轻)
"""
...
EXPERIMENTS = [
    {
        "name": "seashell_masked_V",
        "max_distill_weight": 0.5,
        "desc": "masked L1 only, mw=0.5",
    },
    {
        "name": "seashell_masked_W",
        "max_distill_weight": 0.3,
        "desc": "masked L1 only, mw=0.3",
    },
    {
        "name": "seashell_masked_X",
        "max_distill_weight": 0.1,
        "desc": "masked L1 only, mw=0.1",
    },
]
...
BASE_CFG = {
    ...
    "distill_interval": 8,
    # Masked L1 only: 关掉KL和MSE，开启mask
    "use_kl": False,
    "use_mse": False,
    "use_l1": True,
    "use_mask": True,
...
def main():
    ...
    print(f"\n{'=' * 90}")
    print("Seashell Masked L1 Only 自蒸馏结果")
    print(f"{'=' * 90}")
    print(f"{'实验':<25} {'PSNR':>8} {'SSIM':>10} {'时间':>8}")
    print("-" * 60)
    print(f"{'Baseline':<25} {'40.42':>8} {'0.9428':>10} {'8':>8}")
    print(f"{'自蒸馏Q(L1+KL,mw=0.5)':<25} {'39.81':>8} {'0.9415':>10} {'17':>8}")
    for n, r in results.items():
        print(f"{n:<25} {r['psnr']:>8.2f} {r['ssim']:>10.4f} {r['time_min']:>7.1f}m")
    print("-" * 60)

    with open("experiments/distill/seashell_ablation/results_masked.json", "w") as f:
