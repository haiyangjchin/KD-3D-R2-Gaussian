#!/usr/bin/env python3
"""
Pine 50/75view Full-Distill (A配置: warmup=1000, interval=4, max_weight=0.5, tv=0.1)
用于对比延迟蒸馏 vs 从头蒸馏
"""
...
EXPERIMENTS = [
    {"name": "pine_50v_full_distill_A", "views": 50,
     "desc": "50view Full-Distill A: w=1k, i=4, mw=0.5, tv=0.1"},
    {"name": "pine_75v_full_distill_A", "views": 75,
...
BASE_CFG = {
    ...
    "cnn_model": "./cnn_teachers/pine/checkpoints/checkpoint_epoch_034.pth",
...
def run_one(exp):
    ...
    cfg = dict(BASE_CFG)
    cfg["source_path"] = f"./data/real_dataset/cone_ntrain_{exp['views']}_angle_360/pine"
...
def main():
    ...
    print(f"\n{'='*90}")
    print("Full-Distill vs Delayed-Distill 对比")
    print(f"{'='*90}")
    print(f"{'实验':<30} {'PSNR':>8} {'SSIM':>10} {'时间':>8}")
    print("-" * 65)
    # 参考值
    refs = [
        ("Pine 25v Baseline",      37.69, 0.9276, 7),
        ("Pine 25v Full-Distill A", 38.05, 0.9467, 198),
        ("Pine 25v Delayed-Distill I", 38.73, 0.9289, 9),
        ("Pine 50v Baseline",      39.13, 0.9418, 0),
        ("Pine 50v Delayed-Distill I", 40.69, 0.9421, 9),
        ("Pine 75v Baseline",      39.05, 0.9435, 0),
        ("Pine 75v Delayed-Distill I", 41.02, 0.9436, 9),
    ]
    for n, p, s, t in refs:
        print(f"{n:<30} {p:>8.2f} {s:>10.4f} {t:>7}m")
    for n, r in results.items():
        print(f"{n:<30} {r['psnr']:>8.2f} {r['ssim']:>10.4f} {r['time_min']:>7.1f}m")
    print("-" * 65)

    with open("experiments/distill/pine_ablation/results_full_distill_50_75.json", "w") as f:
