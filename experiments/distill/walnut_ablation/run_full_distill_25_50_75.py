#!/usr/bin/env python3
"""
Walnut 25/50/75view Full-Distill (A配置: warmup=1000, interval=4, max_weight=0.5, tv=0.1)
...
# 顺序执行: 先25v，再50v，最后75v
EXPERIMENTS = [
    {"name": "walnut_25v_full_distill_A", "views": 25,
     "desc": "25view Full-Distill A: w=1k, i=4, mw=0.5, tv=0.1"},
    {"name": "walnut_50v_full_distill_A", "views": 50,
     "desc": "50view Full-Distill A: w=1k, i=4, mw=0.5, tv=0.1"},
    {"name": "walnut_75v_full_distill_A", "views": 75,
...
BASE_CFG = {
    ...
    "cnn_model": "./cnn_teachers/walnut/final_model.pth",
...
def run_one(exp):
    ...
    cfg = dict(BASE_CFG)
    cfg["source_path"] = f"./data/real_dataset/cone_ntrain_{exp['views']}_angle_360/walnut"
...
def main():
    ...
    print(f"\n{'='*90}")
    print("Walnut 25/50/75v Full-Distill 实验结果")
    print(f"{'='*90}")
    ...
    with open("experiments/distill/walnut_ablation/results_full_distill_25_50_75.json", "w") as f:
