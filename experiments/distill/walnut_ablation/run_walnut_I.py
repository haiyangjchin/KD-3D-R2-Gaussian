...
exp = {
    "name": "walnut_exp_I",
    "config": "experiments/distill/pine_ablation/walnut_exp_I.yaml",
    "output": "experiments/distill/pine_ablation/walnut_exp_I",
}
...
print(f"\n{'=' * 70}")
print(
    f"Walnut I:          PSNR={final.get('psnr', 0):.2f}, SSIM={final.get('ssim', 0):.6f}, {elapsed:.1f}min"
)
print(f"Baseline:          PSNR=29.42, SSIM=0.6764, ?min")
print(f"Old distill:       PSNR=29.37, SSIM=0.7263, ?min")
print(f"{'=' * 70}")

with open("experiments/distill/pine_ablation/results_walnut_I.json", "w") as f:
    json.dump(
        {
            "walnut_exp_I": {
