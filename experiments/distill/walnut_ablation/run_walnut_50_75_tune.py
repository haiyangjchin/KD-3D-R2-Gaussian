...
"""
Walnut 50/75view 调参
问题: I配置(mw=0.5, tv=0.2)在50/75view上PSNR下降
策略: 降低蒸馏强度 + 回到原生tv=0.1

实验矩阵 (每组~10min):
  50view:
    M: mw=0.2, tv=0.1, warmup=5000, interval=8  (轻蒸馏+原生TV)
    N: mw=0.1, tv=0.1, warmup=7000, interval=16 (极轻蒸馏+晚介入)
  75view:
    O: mw=0.2, tv=0.1, warmup=5000, interval=8
    P: mw=0.1, tv=0.1, warmup=7000, interval=16
"""
...
EXPERIMENTS = [
    {
        "name": "walnut_50view_M",
        "views": 50,
        "distill_warmup_iters": 5000,
        "distill_interval": 8,
        "max_distill_weight": 0.2,
        "lambda_tv": 0.1,
        "desc": "50v: mw=0.2, tv=0.1, w=5k, i=8",
    },
    {
        "name": "walnut_50view_N",
        "views": 50,
        "distill_warmup_iters": 7000,
        "distill_interval": 16,
        "max_distill_weight": 0.1,
        "lambda_tv": 0.1,
        "desc": "50v: mw=0.1, tv=0.1, w=7k, i=16",
    },
    {
        "name": "walnut_75view_O",
        "views": 75,
        "distill_warmup_iters": 5000,
        "distill_interval": 8,
        "max_distill_weight": 0.2,
        "lambda_tv": 0.1,
        "desc": "75v: mw=0.2, tv=0.1, w=5k, i=8",
    },
    {
        "name": "walnut_75view_P",
        "views": 75,
        "distill_warmup_iters": 7000,
        "distill_interval": 16,
        "max_distill_weight": 0.1,
        "lambda_tv": 0.1,
        "desc": "75v: mw=0.1, tv=0.1, w=7k, i=16",
    },
]
...
def run_one(exp):
    ...
    cfg = dict(BASE_CFG)
    cfg["source_path"] = (
        f"./data/real_dataset/cone_ntrain_{exp['views']}_angle_360/walnut"
    )
...
def main():
    ...
    baselines = {
        "50view baseline": {"psnr": 32.48, "ssim": 0.7188},
        "50view I(mw=0.5,tv=0.2)": {"psnr": 32.34, "ssim": 0.7198},
        "75view baseline": {"psnr": 33.49, "ssim": 0.7297},
        "75view I(mw=0.5,tv=0.2)": {"psnr": 32.99, "ssim": 0.7299},
    }

    print(f"\n{'=' * 95}")
    print("Walnut 50/75view 调参结果")
    print(f"{'=' * 95}")
    print(f"{'实验':<28} {'PSNR':>8} {'SSIM':>10} {'时间':>8}   {'配置'}")
    print("-" * 95)
    for n, b in baselines.items():
        print(f"{n:<28} {b['psnr']:>8.2f} {b['ssim']:>10.4f} {'ref':>8}")
    for n, r in results.items():
        print(
            f"{n:<28} {r['psnr']:>8.2f} {r['ssim']:>10.4f} {r['time_min']:>7.1f}m   {r['desc']}"
        )
    print("-" * 95)

    with open("experiments/distill/walnut_ablation/results_50_75_tune.json", "w") as f:
